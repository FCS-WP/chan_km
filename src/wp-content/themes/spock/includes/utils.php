...
if ( ! function_exists( 'spock_array_get_first' ) ) {
	/**
	 ...
	 */
	function spock_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'spock_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function spock_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'spock_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function spock_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'spock_array_from_list' ) ) {
	/**
	 ...
	 */
	function spock_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'spock_array_slice' ) ) {
	/**
	 ...
	 */
	function spock_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'spock_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * spock_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * spock_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function spock_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'spock_array_insert_after' ) ) {
	/**
	 ...
	 */
	function spock_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = spock_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: spock_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'spock_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function spock_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'spock_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function spock_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'spock_array_str_replace' ) ) {
	/**
	 ...
	 */
	function spock_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = spock_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'spock_generate_id' ) ) {
	/**
	 ...
	 */
	function spock_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'spock_get_tag' ) ) {
	/**
	 ...
	 */
	function spock_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'spock_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function spock_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'spock_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function spock_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( spock_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'spock_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function spock_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'spock_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function spock_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'spock_get_column_class' ) ) {
	/**
	 ...
	 */
	function spock_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'spock_get_svg_from_file' ) ) {
	...
	function spock_get_svg_from_file( $svg ) {
		if ( spock_is_url( $svg ) && ! spock_is_external_url( $svg ) ) {
			$svg_path = spock_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
...
if ( ! function_exists( 'spock_stripslashes' ) ) {
	/**
	 ...
	 */
	function spock_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'spock_get_value_gp' ) ) {
	/**
	 ...
	 */
	function spock_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'spock_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function spock_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'spock_get_value_gps' ) ) {
	/**
	 ...
	 */
	function spock_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'spock_get_session_value' ) ) {
	/**
	 ...
	 */
	function spock_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'spock_set_session_value' ) ) {
	/**
	 ...
	 */
	function spock_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'spock_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function spock_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'spock_get_cookie' ) ) {
	/**
	 ...
	 */
	function spock_get_cookie( $name, $defa = '' ) {
		return spock_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'spock_set_cookie' ) ) {
	/**
	 ...
	 */
	function spock_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'spock_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//spock_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//spock_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'spock' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'spock_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'spock_hex2rgba' ) ) {
	/**
	 ...
	 */
	function spock_hex2rgba( $hex, $alpha ) {
		$rgb = spock_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'spock_rgba2hex' ) ) {
	/**
	 ...
	 */
	function spock_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'spock_hex2hsb' ) ) {
	/**
	 ...
	 */
	function spock_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = spock_rgb2hsb( spock_hex2rgb( $hex ) );
...
if ( ! function_exists( 'spock_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'spock_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'spock_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'spock_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function spock_hsb2hex( $hsb ) {
		return spock_rgb2hex( spock_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'spock_date_to_sql' ) ) {
	/**
	 ...
	 */
	function spock_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'spock_format_price' ) ) {
	/**
	 ...
	 */
	function spock_format_price( $price ) {
...
if ( ! function_exists( 'spock_num2size' ) ) {
	/**
	 ...
	 */
	function spock_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'spock_size2num' ) ) {
	/**
	 ...
	 */
	function spock_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'spock_parse_num' ) ) {
	/**
...
if ( ! function_exists( 'spock_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function spock_prepare_macros( $str ) {
...
if ( ! function_exists( 'spock_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function spock_remove_macros( $str ) {
		return empty( $str ) ? '' : str_replace(
										array( '{{', '}}', '((', '))', '||' ),
										array( '', '', '', '', ' ' ),
										$str
...
if ( ! function_exists( 'spock_is_on' ) ) {
	/**
...
if ( ! function_exists( 'spock_is_off' ) ) {
	/**
...
if ( ! function_exists( 'spock_is_inherit' ) ) {
	/**
	 ...
	 */
	function spock_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'spock_strshort' ) ) {
	/**
	 ...
	 */
	function spock_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = spock_strip_tags( $str );
...
if ( ! function_exists( 'spock_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function spock_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'spock_strip_tags' ) ) {
	/**
...
if ( ! function_exists( 'spock_excerpt' ) ) {
	/**
	 ...
	 */
	function spock_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return spock_strwords( spock_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'spock_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function spock_unserialize_recover( $str ) {
...
if ( ! function_exists( 'spock_unserialize' ) ) {
	/**
	 ...
	 */
	function spock_unserialize( $str ) {
...
if ( ! function_exists( 'spock_str_replace' ) ) {
	/**
	 ...
	 */
	function spock_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = spock_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( spock_str_replace( $from, $to, spock_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'spock_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function spock_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'spock_tags_count' ) ) {
	/**
	 ...
	 */
	function spock_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'spock_getimagesize' ) ) {
	/**
	 ...
	 */
	function spock_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = spock_is_url( $url ) ? spock_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'spock_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function spock_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = spock_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'spock_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function spock_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = spock_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = spock_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = spock_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = spock_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'spock_get_thumb_size' ) ) {
	 ...
	 */
	function spock_get_thumb_size( $ts ) {
		$thumb_sizes = spock_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "spock-thumb-{$ts}" ] ) ) {
			$ts = 'spock-thumb-' . $ts;
		}
		$retina = spock_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'spock_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'spock_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'spock-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'spock_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'spock-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'spock_get_closest_thumb_size' ) ) {
	 ...
	 */
	function spock_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'spock_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function spock_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = spock_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'spock_get_post_image' ) ) {
	/**
	 ...
	 */
	function spock_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'spock_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function spock_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = spock_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = spock_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = spock_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( spock_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = spock_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'spock_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function spock_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'spock_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'spock_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function spock_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = spock_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = spock_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'spock_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'spock_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function spock_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'spock_get_post_video_list_first' ) ) {
	/**
...
if ( ! function_exists( 'spock_get_post_iframe' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function spock_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = spock_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = spock_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'spock_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'spock_get_embed_video' ) ) {
	/**
	...
	function spock_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = spock_add_to_url( $video, spock_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'spock_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'spock_filter_video_width', 325, 'tiktok' ) : spock_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'spock_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'spock_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function spock_get_embed_video_tag_name() {
...
if ( ! function_exists( 'spock_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function spock_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = spock_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										spock_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'spock_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function spock_is_from_uploads( $url ) {
		$url          = spock_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = spock_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'spock_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function spock_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'spock_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function spock_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'spock_mkdir' ) ) {
	/**
...
if ( ! function_exists( 'spock_unlink' ) ) {
	/**
	 ...
	 */
	function spock_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
...
if ( ! function_exists( 'spock_unzip_file' ) ) {
	/**
	 ...
	 * @return bool         Return true if no error occurs.
	 */
	function spock_unzip_file( $zip, $dest ) {
...
if ( ! function_exists( 'spock_retrieve_json' ) ) {
	/**
...
if ( ! function_exists( 'spock_esc' ) ) {
	 ...
	 */
	function spock_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'spock_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'spock_check_min_file' ) ) {
	 ...
	 */
	function spock_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && spock_is_off( spock_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && spock_is_off( spock_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'spock_get_file_dir' ) ) {
	/**
	 ...
	 */
	function spock_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! SPOCK_ALLOW_SKINS && ! spock_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'spock_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( SPOCK_CHILD_DIR != SPOCK_THEME_DIR && file_exists( SPOCK_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? SPOCK_CHILD_URL : SPOCK_CHILD_DIR ) . spock_check_min_file( $file, SPOCK_CHILD_DIR );
			} elseif ( file_exists( SPOCK_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? SPOCK_THEME_URL : SPOCK_THEME_DIR ) . spock_check_min_file( $file, SPOCK_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'spock_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function spock_get_file_url( $file ) {
		return spock_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'spock_get_file_ext' ) ) {
	/**
	 ...
	 */
	function spock_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'spock_get_file_name' ) ) {
	/**
	 ...
	 */
	function spock_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'spock_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function spock_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'spock_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( SPOCK_CHILD_DIR != SPOCK_THEME_DIR && is_dir( SPOCK_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? SPOCK_CHILD_URL : SPOCK_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( SPOCK_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? SPOCK_THEME_URL : SPOCK_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'spock_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'spock_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function spock_get_folder_url( $folder ) {
		return spock_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'spock_merge_js' ) ) {
	/**
	 ...
	 */
	function spock_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= spock_fgc( spock_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = spock_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = spock_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			spock_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'spock' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'spock_filter_js_output', apply_filters( 'spock_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'spock_merge_css' ) ) {
	/**
	 ...
	 */
	function spock_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'spock_filter_responsive_sizes', spock_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = spock_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = spock_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'spock_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'spock_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'spock' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'spock' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = spock_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			spock_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'spock') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'spock_filter_css_output', apply_filters( 'spock_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'spock_filter_merge_css' ) ) {
	add_filter( 'spock_filter_merge_css', 'spock_filter_merge_css' );
	/**
	 ...
	 */
	function spock_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'spock_merge_sass' ) ) {
	/**
	 ...
	 */
	function spock_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'spock_filter_responsive_sizes', spock_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'spock_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			spock_fpc( spock_get_file_dir( $to ), apply_filters( 'spock_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'spock_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function spock_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'spock_add_protocol' ) ) {
	/**
	 ...
	 */
	function spock_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? spock_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'spock_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function spock_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'spock_add_to_url' ) ) {
	/**
	 ...
	 */
	function spock_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = spock_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'spock_is_url' ) ) {
	/**
	 ...
	 */
	function spock_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'spock_is_external_url' ) ) {
	/**
	 ...
	 */
	function spock_is_external_url( $url ) {
		...
		return spock_is_url( $url ) && strpos( $url, spock_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}

if ( ! function_exists( 'spock_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function spock_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => spock_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => SPOCK_CHILD_DIR,
				'url' => spock_remove_protocol_from_url( SPOCK_CHILD_URL )
				),
			'theme' => array(
				'dir' => SPOCK_THEME_DIR,
				'url' => spock_remove_protocol_from_url( SPOCK_THEME_URL )
				)
			);
