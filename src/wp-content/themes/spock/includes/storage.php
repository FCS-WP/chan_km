...
if ( ! function_exists( 'spock_storage_get' ) ) {
	/**
	 ...
	 */
	function spock_storage_get( $var_name, $default = '' ) {
		global $SPOCK_STORAGE;
		return isset( $SPOCK_STORAGE[ $var_name ] ) ? $SPOCK_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'spock_storage_set' ) ) {
	/**
	 ...
	 */
	function spock_storage_set( $var_name, $value ) {
		global $SPOCK_STORAGE;
		$SPOCK_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'spock_storage_empty' ) ) {
	/**
	 ...
	 */
	function spock_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $SPOCK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $SPOCK_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $SPOCK_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'spock_storage_isset' ) ) {
	/**
	 ...
	 */
	function spock_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $SPOCK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $SPOCK_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $SPOCK_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'spock_storage_unset' ) ) {
	/**
	 ...
	 */
	function spock_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $SPOCK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $SPOCK_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $SPOCK_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $SPOCK_STORAGE[ $var_name ] ) ) {
				unset( $SPOCK_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'spock_storage_inc' ) ) {
	/**
	 ...
	 */
	function spock_storage_inc( $var_name, $value = 1 ) {
		global $SPOCK_STORAGE;
		if ( empty( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = 0;
		}
		$SPOCK_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'spock_storage_concat' ) ) {
	/**
	 ...
	 */
	function spock_storage_concat( $var_name, $value ) {
		global $SPOCK_STORAGE;
		if ( empty( $SPOCK_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'spock_storage_get_array' ) ) {
	/**
	 ...
	 */
	function spock_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $SPOCK_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $SPOCK_STORAGE[ $var_name ][ $key ] ) ? $SPOCK_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'spock_storage_set_array' ) ) {
	/**
	 ...
	 */
	function spock_storage_set_array( $var_name, $key, $value ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $SPOCK_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$SPOCK_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'spock_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function spock_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $SPOCK_STORAGE[ $var_name ][ $key ] ) ) {
			$SPOCK_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $SPOCK_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$SPOCK_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$SPOCK_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'spock_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function spock_storage_merge_array( $var_name, $key, $value ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$SPOCK_STORAGE[ $var_name ] = array_merge( $SPOCK_STORAGE[ $var_name ], $value );
		} else {
			$SPOCK_STORAGE[ $var_name ][ $key ] = array_merge( $SPOCK_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'spock_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function spock_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			spock_array_insert_after( $SPOCK_STORAGE[ $var_name ], $after, $key );
		} else {
			spock_array_insert_after( $SPOCK_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'spock_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function spock_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			spock_array_insert_before( $SPOCK_STORAGE[ $var_name ], $before, $key );
		} else {
			spock_array_insert_before( $SPOCK_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'spock_storage_push_array' ) ) {
	/**
	 ...
	 */
	function spock_storage_push_array( $var_name, $key, $value ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $SPOCK_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $SPOCK_STORAGE[ $var_name ][ $key ] ) ) {
				$SPOCK_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $SPOCK_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'spock_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function spock_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $SPOCK_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $SPOCK_STORAGE[ $var_name ] ) && is_array( $SPOCK_STORAGE[ $var_name ] ) && count( $SPOCK_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $SPOCK_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $SPOCK_STORAGE[ $var_name ][ $key ] ) && is_array( $SPOCK_STORAGE[ $var_name ][ $key ] ) && count( $SPOCK_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $SPOCK_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'spock_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function spock_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( empty( $SPOCK_STORAGE[ $var_name ][ $key ] ) ) {
			$SPOCK_STORAGE[ $var_name ][ $key ] = 0;
		}
		$SPOCK_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'spock_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function spock_storage_concat_array( $var_name, $key, $value ) {
		global $SPOCK_STORAGE;
		if ( ! isset( $SPOCK_STORAGE[ $var_name ] ) ) {
			$SPOCK_STORAGE[ $var_name ] = array();
		}
		if ( empty( $SPOCK_STORAGE[ $var_name ][ $key ] ) ) {
			$SPOCK_STORAGE[ $var_name ][ $key ] = '';
		}
		$SPOCK_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'spock_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function spock_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $SPOCK_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $SPOCK_STORAGE[ $var_name ] ) ? $SPOCK_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $SPOCK_STORAGE[ $var_name ] ) ? $SPOCK_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'spock_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function spock_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $SPOCK_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $SPOCK_STORAGE[ $var_name ]->$prop ) ? $SPOCK_STORAGE[ $var_name ]->$prop : $default;
	}
}
