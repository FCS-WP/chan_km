...
if ( function_exists( 'spock_elementor_is_preview' ) && spock_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			spock_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	spock_new_main_query(
		array(
			'post_type'      => spock_get_theme_option( 'post_type' ),
			'category'       => spock_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'spock_filter_get_template_part', spock_blog_archive_get_template() ) );
}
