...
if ( is_array( $spock_template_args ) ) {
	...
	if ( ! empty( $spock_template_args['slider'] ) ) {
		...
		?>
		<div class="<?php echo esc_attr( $spock_columns_class ); ?>">
		<?php
	}
} else {
...
<article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>"
	<?php
	post_class( 'post_item post_item_container post_layout_excerpt post_format_' . esc_attr( $spock_post_format ) );
...
>
	...
	spock_show_post_featured( apply_filters( 'spock_filter_args_featured',
		array(
			...
			'thumb_size' => ! empty( $spock_template_args['thumb_size'] )
							...
							: spock_get_thumb_size( strpos( spock_get_theme_option( 'body_style' ), 'full' ) !== false
								? 'full'
								: ( $spock_expanded 
									? 'huge' 
									: 'big' 
	...
	if ( $spock_show_title ) {
		...
		<div class="post_header entry-header">
			<?php
			// Post title
			if ( apply_filters( 'spock_filter_show_blog_title', true, 'excerpt' ) ) {
	...
	// Post content
	if ( apply_filters( 'spock_filter_show_blog_excerpt', empty( $spock_template_args['hide_excerpt'] ) && spock_get_theme_option( 'excerpt_length' ) > 0, 'excerpt' ) ) {
		?>
		<div class="post_content entry-content">
			<?php

			// Post meta
			if ( apply_filters( 'spock_filter_show_blog_meta', $spock_show_meta, $spock_components, 'excerpt' ) ) {
			...
			if ( spock_get_theme_option( 'blog_content' ) == 'fullpost' ) {
				// Post content area
				?>
			...
			// More button
			if ( apply_filters( 'spock_filter_show_blog_readmore',  ! isset( $spock_template_args['more_button'] ) || ! empty( $spock_template_args['more_button'] ), 'excerpt' ) ) {
				if ( empty( $spock_template_args['no_links'] ) ) {
					do_action( 'spock_action_before_post_readmore' );
					if ( spock_get_theme_option( 'blog_content' ) != 'fullpost' ) {
						spock_show_post_more_link( $spock_template_args, '<p>', '</p>' );
					} else {
						spock_show_post_comments_link( $spock_template_args, '<p>', '</p>' );
					}
