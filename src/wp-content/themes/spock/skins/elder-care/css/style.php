...
if ( ! function_exists( 'spock_customizer_get_css' ) ) {

	function spock_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = spock_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.spock_tabs .spock_tabs_titles li a {
...
.wpgdprc, .author_info .author_label,
...
CSS;
			$rez          = apply_filters( 'spock_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = spock_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'spock_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( spock_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = spock_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'spock_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'spock_filter_prepare_css', $css_str, $remove_spaces );
	}
}
