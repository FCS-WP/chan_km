<div class="front_page_section front_page_section_testimonials<?php
	$spock_scheme = spock_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $spock_scheme ) && ! spock_is_inherit( $spock_scheme ) ) {
		echo ' scheme_' . esc_attr( $spock_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( spock_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( spock_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$spock_css      = '';
		$spock_bg_image = spock_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$spock_anchor_icon = spock_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$spock_anchor_text = spock_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( spock_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$spock_css      = '';
			$spock_bg_mask  = spock_get_theme_option( 'front_page_testimonials_bg_mask' );
			$spock_bg_color_type = spock_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $spock_bg_color_type ) {
				$spock_bg_color = spock_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$spock_caption = spock_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $spock_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $spock_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $spock_caption, 'spock_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$spock_description = spock_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $spock_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $spock_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $spock_description ), 'spock_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! spock_exists_trx_addons() ) {
						...
						spock_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
