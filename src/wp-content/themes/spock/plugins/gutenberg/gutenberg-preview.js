...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			spock_create_observer( 'spock-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				spock_gutenberg_editor_init();
			} );
		} else {
		 	spock_gutenberg_editor_init();
		}
	...
	function spock_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'spock_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'spock_inited' ) ) {
	...
	// Init on page load
	function spock_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = spock_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + SPOCK_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + SPOCK_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in SPOCK_STORAGE['override_classes'] ) {
				$obj.addClass( SPOCK_STORAGE['override_classes'][i].replace( '%s', SPOCK_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( spock_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && SPOCK_STORAGE['sidebar_position'] != 'hide' ) {
				if ( spock_apply_filters( 'spock_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'spock_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( spock_apply_filters( 'spock_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				spock_remove_observer( 'spock-check-editor-styles-wrapper' );
				spock_create_observer( 'spock-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'spock_inited' );
	}
} );
