...
jQuery( document ).ready(
	function() {
		...
		spock_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="spock_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.spock_post_editor' );
						if ( link.length > 0 ) {
							spock_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.spock_post_editor', function(e) {
						spock_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('spock_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function spock_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
